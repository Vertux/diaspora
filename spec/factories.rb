...
FactoryGirl.define do
  ...
  factory :unsearchable_person, :parent => :person do |p|
    p.after_build do |p|
  ...
  factory(:status_message_with_photo, :parent => :status_message) do
    ...
    after_build do |sm|
      Factory(:photo, :author => sm.author, :status_message => sm, :pending => false, :public => sm.public)
    end
  end
  ...
  factory :oauth_client_permission do
  end

 factory :app, :class => OAuth2::Provider.client_class do |a|
  ...
  factory :oauth_authorization_code, :class => OAuth2::Provider.authorization_code_class_name do |a|
    a.redirect_uri "http://localhost/callback"
    a.association(:authorization, :factory => :oauth_authorization)
  end

  factory :oauth_access_token, :class => OAuth2::Provider.access_token_class do |a|
    a.association(:authorization, :factory => :oauth_authorization)
  end

  factory :tag, :class => ActsAsTaggableOn::Tag do
    sequence(:name) { |n| "partytimeexcellent#{n}" }
  end
